...
public:
  ...
  bool load(int pdg, const std::string& filename) {
    ...
    if (it == mPdgToIndexMap.end()) {
        ...
        // Resize flattened arrays if needed
        size_t totalSize = (current_pdg_idx + 1) * mMaxLayers * mEtaBins * mPtBins;
        if (mLUTDataFlat.size() < totalSize * 2) { // *2 for MPV and resolution
            mLUTDataFlat.resize(totalSize * 2, -1.f);
        }
      ...
      for (int etaBin = 0; etaBin < mEtaBins; ++etaBin) {
        ...
        for (int ptBin = 0; ptBin < mPtBins; ++ptBin) {
          ...
          if (hist_from_file) {
            ...
            mLUTHistogramFlat[flatIdx / 2] = cloned_hist; // Divide by 2 since histogram array is not doubled
            ...
            if (cloned_hist->GetEntries() > 50) {
              ...
              mLUTDataFlat[flatIdx] = mpv;         // MPV
              mLUTDataFlat[flatIdx + 1] = resolution; // Resolution
            } else {
              mLUTDataFlat[flatIdx] = -1.f;
              mLUTDataFlat[flatIdx + 1] = -1.f;
            }
          } else {
            size_t flatIdx = getFlatIndex(current_pdg_idx, layer, etaBin, ptBin);
            mLUTHistogramFlat[flatIdx / 2] = nullptr;
            mLUTDataFlat[flatIdx] = -1.f;
            mLUTDataFlat[flatIdx + 1] = -1.f;
            success = false;
          }
  ...
  TH1F* getHistogramForSampling(int pdg_idx, int layer, int etaBin, int ptBin) const {
    ...
    size_t flatIdx = getFlatIndex(pdg_idx, layer, etaBin, ptBin) / 2;
  ...
  float getMPV(int pdg_idx, int layer, int etaBin, int ptBin) const {
    ...
    return (flatIdx < mLUTDataFlat.size()) ? mLUTDataFlat[flatIdx] : -1.f;
  }
  ...
  float getResolution(int pdg_idx, int layer, int etaBin, int ptBin) const {
    ...
    size_t flatIdx = getFlatIndex(pdg_idx, layer, etaBin, ptBin);
    return (flatIdx + 1 < mLUTDataFlat.size()) ? mLUTDataFlat[flatIdx + 1] : -1.f;
  }
  ...
  // Flattened index calculation for cache-friendly access
  inline size_t getFlatIndex(int pdgIdx, int layer, int etaBin, int ptBin, int valueIdx = 0) const {
    return (((pdgIdx * mMaxLayers + layer) * mEtaBins + etaBin) * mPtBins + ptBin) * 2 + valueIdx;
  }

  // Get total number of loaded PDG types
  size_t getNumPdgTypes() const {
    return mIndexToPdgMap.size();
  }

  // Flattened data structure for better cache locality
  std::vector<float> mLUTDataFlat; // [pdg][layer][eta][p][mpv/resolution]
  std::vector<TH1F*> mLUTHistogramFlat; // Flattened histogram storage
  
  // Fast lookup tables
  std::unordered_map<int, int> mPdgToIndexMap; // unordered_map is faster than map
  ...
  float mEtaMin;
  float mEtaMax;
  int mPtBins;      // Number of pT bins (LUTs are pT-based)
  float mPtMin;     // Minimum pT value for LUT binning
  float mPtMax;     // Maximum pT value for LUT binning
  
  // Cached bin calculation helpers
  float mEtaBinWidth;
  float mPtBinWidth;
...
static constexpr int kNumHypothesisParticles = 9; // Updated to support 9 particle types (including muons): el, mu, pi, ka, pr, de, tr, he3, al
...
struct OnTheFlyTrackerPid {
  ...
  void process(soa::Join<aod::Collisions, aod::McCollisionLabels>::iterator const& collision,
    ...
    for (const auto& track : tracks) {
      // Initialize default values for tracks that fail validation
      float truncatedMeanToT = -1.0f;
      std::array<float, kNumHypothesisParticles> nSigmaValues;
      nSigmaValues.fill(999.f);

      if (!track.has_mcParticle()) {
        // Write invalid entries but continue to maintain table size consistency
        ...
        tableUpgradeTrkPids(nSigmaValues[0], nSigmaValues[1], nSigmaValues[2], nSigmaValues[3],
                           nSigmaValues[4], nSigmaValues[5], nSigmaValues[6], nSigmaValues[7], nSigmaValues[8]); // 9 values
        continue;
      }
      ...
      const int actualPdgCode = mcParticle.pdgCode();
      const int truePdgCode = std::abs(mcParticle.pdgCode());

      // Calculate rigidity (p/z) using MC truth charge
      float rigidity = p; // fallback to momentum if charge unknown
      if (pdgInfo) {
      ...
      if (true_pdg_idx == -1) {
          ...
          tableUpgradeTrkPidSignals(truncatedMeanToT);
          tableUpgradeTrkPids(nSigmaValues[0], nSigmaValues[1], nSigmaValues[2], nSigmaValues[3],
                             nSigmaValues[4], nSigmaValues[5], nSigmaValues[6], nSigmaValues[7], nSigmaValues[8]); // 9 values (including muon)
          continue;
      }
      ...
      // Use optimized bin calculation - use transverse momentum pT for LUT lookup (LUTs are pT-based)
      const int binnedPt = mToTLUT->getPtBin(pt);
      const int binnedEta = mToTLUT->getEtaBin(eta);
      ...
      for (int layer = 3; layer < mMaxBarrelLayers.value; ++layer) {
        if ((hitMap >> layer) & 0x1) {
          ...
          if (totHist && totHist->GetEntries() > 50) {
      ...
      // Reset truncatedMeanToT for this track (already declared at loop start)
      ...
      // Reset nSigma values for this track (already declared at loop start)
      nSigmaValues.fill(999.f);
